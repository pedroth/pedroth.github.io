...
function mouseMove(e) {
  ...
  if (!down || (mx == mouse[0] && my == mouse[1])) return;
  mouse[0] = my;
  mouse[1] = mx;
}
  ...
  while (x[0] !== x2[0] || x[1] !== x2[1]) {
    ...
    for (var i = -size; i < size + 1; i++) {
      for (var j = -size; j < size + 1; j++) {
        ...
        var res =
          shadePow + Math.abs(nextX[0] - x2[0]) + Math.abs(nextX[1] - x2[1]);
...
function draw() {
  ...
  if (animationFrame.isReady) {
    ...
    drawImageData(data, animationFrame.data, mouse, imageSize, function (x) {
