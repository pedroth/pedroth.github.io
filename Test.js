	...
	while (x[0] !== x2[0] || x[1] !== x2[1]) {
		...
		for ( var i = -1; i < 2; i++) {
			for ( var j = -1; j < 2; j++) {
				...
				var nextX = add(x, [ i, j ]);
				var shadePow = Math.abs(innerProd(diff(nextX, x1), normal)); 
				data[4 * height * x[0] + 4 * x[1]    ] = Math.floor(clamp(r * (1 - shadePow / maxShade),0,255));
				data[4 * height * x[0] + 4 * x[1] + 1] = Math.floor(clamp(g * (1 - shadePow / maxShade),0,255));
				data[4 * height * x[0] + 4 * x[1] + 2] = Math.floor(clamp(b * (1 - shadePow / maxShade),0,255));
			}
		}
...
function draw() {
	...
	if (numOfPoints > 1) {
		...
		drawLine(points[0], myI, 1, data, 255, 255, 255);
	}
